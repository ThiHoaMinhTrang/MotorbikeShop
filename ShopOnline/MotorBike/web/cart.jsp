...
<html class="no-js" lang="zxx">
    ...
    <li><a href="login.jsp">Login</a></li>
        ...
        %>
    <li><a href="myAccount.jsp">Welcome <%= loginUser.getFullName()%></a></li>
        <%
            }
        %>
    <li class="last">
        <a href="phoneAll.jsp"><img src="images/search_icon.png" alt="icon" /></a>
    </li>
    <%
            CartDTO cart = (CartDTO) session.getAttribute("CART");
            if (cart != null) {
        %>
        <div id="table" style="text-align: center; display: flex; justify-content: center;align-items: center">
            <table border="1">
                <thead>
                    <tr>
                        <th>No</th>
                ...
                <tbody>
                    <%
                        ...
                        for (ProductDTO p : cart.getCart().values()) {
                            total += p.getPrice() * p.getQuantity() * ((100 - (double) p.getDiscount()) / 100);
                    %>
                <form action="MainController" method="POST">
                    <tr>
                        ...
                        <td><%= p.getItemName()%></td>
                        <td><%= p.getBrand()%></td>                          
                        <td><%= (float) p.getPrice()%>$</td>
                        <td>
                            <input min="1" type="number" name="quantity" value="<%= p.getQuantity()%>" />
                        </td>
                        <td><%= p.getDiscount()%>%</td>
                        <td><%= Math.round((p.getPrice() * p.getQuantity() * (double) ((100.0 - (double) p.getDiscount()) / 100.0)) * 100.0) / 100.0%>$</td>
                        <td>
